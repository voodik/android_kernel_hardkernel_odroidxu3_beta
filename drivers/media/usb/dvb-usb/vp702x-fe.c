...
static int vp702x_fe_read_status(struct dvb_frontend* fe, enum fe_status *status)
{
...
static int vp702x_fe_send_diseqc_burst (struct dvb_frontend* fe, enum fe_sec_mini_cmd burst)
{
	deb_fe("%s\n",__func__);
	return 0;
}

static int vp702x_fe_set_tone(struct dvb_frontend* fe, enum fe_sec_tone_mode tone)
{
	struct vp702x_fe_state *st = fe->demodulator_priv;
	struct vp702x_device_state *dst = st->d->priv;
	u8 *buf;
...
static int vp702x_fe_set_voltage (struct dvb_frontend* fe, enum fe_sec_voltage
		voltage)
{
	struct vp702x_fe_state *st = fe->demodulator_priv;
	struct vp702x_device_state *dst = st->d->priv;
