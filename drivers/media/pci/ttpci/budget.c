 ...
 */
static int SetVoltage_Activy (struct budget *budget, enum fe_sec_voltage voltage)
{
	struct saa7146_dev *dev=budget->dev;
...
static int siemens_budget_set_voltage(struct dvb_frontend* fe, enum fe_sec_voltage voltage)
{
...
static int budget_set_tone(struct dvb_frontend* fe, enum fe_sec_tone_mode tone)
{
	struct budget* budget = (struct budget*) fe->dvb->priv;
...
static int budget_diseqc_send_burst(struct dvb_frontend* fe, enum fe_sec_mini_cmd minicmd)
{
	struct budget* budget = (struct budget*) fe->dvb->priv;
