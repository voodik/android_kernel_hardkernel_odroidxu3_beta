...
#if defined(CONFIG_MACH_ODROIDXU3)
	par->reg_gpx1 = ioremap(ODROIDXU3_GPX1_REG, 4);
	par->reg_gpx2 = ioremap(ODROIDXU3_GPX2_REG, 4);
	par->reg_gpa2 = ioremap(ODROIDXU3_GPA2_REG, 4);
...
static int flexfb_remove_common(struct device *dev, struct fb_info *info)
{
	...
	if (par)
		fbtft_par_dbg(DEBUG_DRIVER_INIT_FUNCTIONS, par,
			"%s()\n", __func__);
#if defined(CONFIG_MACH_ODROIDXU3)
	if (par->reg_gpx1)	iounmap(par->reg_gpx1);
	if (par->reg_gpx2)	iounmap(par->reg_gpx2);
	if (par->reg_gpa2)	iounmap(par->reg_gpa2);
#endif
