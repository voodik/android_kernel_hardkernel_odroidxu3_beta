 ...
 * CPUMAPPED: set when buffer is mapped to cpu
 * CLEAN: set when buffer is clean
 * READY_TO_USE: set when buffer is initialized
 */
#define ION_PRIV_FLAG_CPUMAPPED (1 << 15)
#define ION_PRIV_FLAG_CLEAN (1 << 14)
#define ION_PRIV_FLAG_READY_TO_USE (1 << 13)
...
{
	buffer->private_flags |= ION_PRIV_FLAG_CPUMAPPED;
	if (buffer->flags & ION_FLAG_CACHED)
		buffer->private_flags &= ~ION_PRIV_FLAG_CLEAN;
}
...
static inline bool ion_buffer_dirty(struct ion_buffer *buffer)
{
	...
	if ((buffer->flags & ION_FLAG_CACHED) &&
		...
		return !(buffer->private_flags & ION_PRIV_FLAG_CLEAN);
}
...
static inline void ion_buffer_make_ready(struct ion_buffer *buffer)
{
	if (!(buffer->private_flags & ION_PRIV_FLAG_READY_TO_USE)) {
		...
		buffer->private_flags |= ION_PRIV_FLAG_READY_TO_USE;
		if (!(buffer->private_flags & ION_PRIV_FLAG_CPUMAPPED))
			buffer->private_flags |= ION_PRIV_FLAG_CLEAN;
	}
}
