...
//
//------------------------------------------------------
#define HARDKERNEL_PATCH

#if defined(HARDKERNEL_PATCH)
    #define VSYNC_TIMEOUT_MSEC 10
#else
...
int create_decon_display_controller(struct platform_device *pdev)
{
    ...
    {
        struct s3c_fb_platdata *fimd_config;
        int i;

        fimd_config = dispdrv->dt_ops.get_display_platdata();
    	for (i = 0; i < S3C_FB_MAX_WIN; i++)
            pd->win[i] = fimd_config->win[i];
    }
#else	    
...
#endif
#ifdef CONFIG_FB_I80_COMMAND_MODE
	/* Set WINMAP(window 0) Black */
	writel(0x1000000, sfb->regs + WINxMAP(0));
	s3c_fb_hw_trigger_set(sfb, TRIG_UNMASK);
	s5p_mipi_dsi_wr_data(dsim_for_decon, MIPI_DSI_DCS_SHORT_WRITE,
		0x29, 0);
#endif
